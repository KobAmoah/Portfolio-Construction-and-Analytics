...
cum.rets.chart<-highchart(type='stock')%>%
                ...
                hc_add_series(round(combined.rets[,1],2), 
                              name=colnames(combined.rets)[1])%>%
                hc_add_series(round(combined.rets[,2],2), 
                               name=colnames(combined.rets)[2])%>%
                hc_add_series(round(combined.rets[,3],2), 
                              name=colnames(combined.rets)[3])%>%
                hc_add_series(round(combined.rets[,4],2), 
...
key.measures<- function( Ra, port.weight, bm, scale = 12,Sigma = cov(etf.rets), Rf= 0){
  ...
  res<-    round( rbind(ret,act.ret,ex.ret, st.dev,beta,sharpe,
                        treynor,m.square,i.ratio, t.error, dr,cr),3)
...
print("Key Portfolio Performance Measures")
all.ports
