...
etf.symbols <- c("SPY", "IWM", "AGG", "FEZ", "ACWI", "IYR","REET","GSG","IAGG")
...
# The following function obtained from Ross Bennett(2018) Paper
...
BL.weights<-optimize.portfolio(R = etf.rets,portfolio= port.spec,maxSR=TRUE,
...
# Create List of portfolios objects for display their weights
...
combined.rets<- cbind(GMV.rets$Returns,EW.rets$Returns,RO.rets$Returns,BL.rets$Returns) *10000
...
cum.rets.chart<-highchart(type='stock')%>%
                hc_title(text='Hypotheical Growth of $10,000 - Invested in Dec, 2021') %>%
...
key.measures<- function( Ra, port.weight,  bm, scale = 12,Sigma = cov(etf.rets)){
  ...
  sharpe<-  SharpeRatio.annualized(Ra, Rf=0, scale = scale)
  ...
  res<-     rbind(ret,act.ret,st.dev,sharpe, treynor,i.ratio,m.square, t.error, dr,cr) 
  rownames(res) <- c("Return(%)","Active Return(%)",'Standard Deviation(%)','Sharpe','Treynor',
...
# Calculate Returns and remove na value in first row
# Select Returns from 2022 onwards
